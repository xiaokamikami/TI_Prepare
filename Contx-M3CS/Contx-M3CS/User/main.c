...
void Mian_Init(void);
void Dock_angle(uint8_t angle);	//180为最右  360最左
...
{	
	...
	if(mode == 1)
	{
		...
		sprintf((char * )LCD_Buff,"L1_SPEED_SET:%d , R1_SPEED_SET:%d   ",speed1,speed2);
...
int main(void)
{
	...
	LCD_ShowString(10*8,1*16,(u8*)"Found Ball ......  ",WHITE);
	while(1)
	{	
...
void Mian_Init(void)
{
	...
	while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_13) == 1){} //按键开始自检
		delay_ms(200);
	//**PWM Open
	//TIM8_PWM_Init(719, 1000); //1000 * 720 / 72MHz = 10MS = 1000HZ
